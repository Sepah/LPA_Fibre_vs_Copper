...
{	
    ...
    ddm_type = ((int) A50[92] << 8);
        for (int i=0; i<8; i++) 
    	{
			printf("%d ", ddm_type >> i & 1);		
		}
		printf("DDM Type - Byte Descriptor: %d\n",ddm_type);
...
int control_on(void)
{
		...
		//activate solenoid by providing 3.3v power to
		//pin 11 of the header
        pinMode(GPIO_pinon, OUTPUT);
		digitalWrite(GPIO_pinon, 1);  // power on solenoid
		
		printf("'ON' Solenoid activated");
		
		delay(1500);			  	  // wait 1.5 sec
		digitalWrite(GPIO_pinon, 0);  // power off solenoid
		
		printf(" and dectivated\n");
        return 0;
}
...
int control_off(void)
{
		...
		digitalWrite(GPIO_pinoff, 1);  // power on solenoid
		
		printf("'OFF' Solenoid activated ");
		
		delay(1500);			  	   // wait 1.5 sec
		digitalWrite(GPIO_pinoff, 0);  // power off solenoid
...
int control_break(void)
{
	...
	printf("Please identify the broken cable and then remove\n");
	userans = 0;
	...
	//Connect to GPIO as with solenoids
	delay(2000);
	switch (userans)
	{
		case 1:
